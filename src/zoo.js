...
const horario = (dia) => {
  const str = `Open from ${hours[dia].open}am until ${hours[dia].close - 12}pm`;
  return str;
};
...
function getSchedule(dayName) {
  ...
  if (dayName === undefined) {
    for (let index = 0; index < arr.length - 1; index += 1) {
      objOrd[arr[index]] = horario(arr[index]);
    }
    objOrd.Monday = 'CLOSED';
    return objOrd;
  }
  if (dayName === 'Monday') {
    objOrd.Monday = 'CLOSED';
  ...
  objOrd[dayName] = horario(dayName);
  return objOrd;
}
...
function increasePrices(percentage) {
  ...
  const crianca = ((prices.Child * percentage) / 100) + prices.Child;
  const idoso = ((prices.Senior * percentage) / 100) + prices.Senior;
